...
(function () {
  ...
  /* En un telefono la tesis y el coste no se clavan: se leen enteras. El
     resto del movimiento sigue igual, asi que no vale el modo plano. */
  var enMovil = window.matchMedia('(max-width:760px)').matches;

  function resolverTesis() {
    document.querySelectorAll('.tesis__l').forEach(function (e) {
      e.style.setProperty('--w', '100%');
    });
    document.querySelectorAll('.tesis__fl path').forEach(function (e) {
      e.style.strokeDashoffset = 0;
    });
    ['tesisV', 'tesisPie'].forEach(function (id) {
      var e = document.getElementById(id);
      if (e) { e.style.opacity = 1; e.style.transform = 'none'; }
    });
  }

  function resolverCoste() {
    document.querySelectorAll('.coste2__uno').forEach(function (f) {
      f.classList.add('esta');
      f.style.opacity = 1;
      f.style.transform = 'none';
    });
    document.querySelectorAll('.coste2__uno .barra i, .coste2__uno .escala i')
      .forEach(function (e) { e.style.transform = 'scaleX(1)'; });
    document.querySelectorAll('.coste2__uno .ocho i, .coste2__uno .malla25 i')
      .forEach(function (e) { e.style.opacity = 1; });
  }
  ...
  if (enMovil) { resolverTesis(); resolverCoste(); }
